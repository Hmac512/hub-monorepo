...
describe("connectionFilter tests", () => {
  ...
  test("denies all connections by default", async () => {
    ...
    await expect(
      filter.denyDialMultiaddr(multiaddr(allowedMultiAddrStr))
    ).resolves.toBeTruthy();
    // Incepient Inbound Connections are always allowed
    await expect(
      filter.denyInboundConnection(remoteConnection as MultiaddrConnection)
    ).resolves.toBeFalsy();
    await expect(
      filter.denyInboundEncryptedConnection(
        allowedPeerId,
        remoteConnection as MultiaddrConnection
      )
    ).resolves.toBeTruthy();
    await expect(
      filter.denyInboundUpgradedConnection(
        allowedPeerId,
        remoteConnection as MultiaddrConnection
      )
    ).resolves.toBeTruthy();
    await expect(
      filter.denyOutboundConnection(
        allowedPeerId,
        remoteConnection as MultiaddrConnection
      )
    ).resolves.toBeTruthy();
    await expect(
      filter.denyOutboundEncryptedConnection(
        allowedPeerId,
        remoteConnection as MultiaddrConnection
      )
    ).resolves.toBeTruthy();
    await expect(
      filter.denyOutboundUpgradedConnection(
        allowedPeerId,
        remoteConnection as MultiaddrConnection
      )
    ).resolves.toBeTruthy();
    await expect(
      filter.filterMultiaddrForPeer(allowedPeerId)
    ).resolves.toBeFalsy();
  });
  ...
  test("filters unknown peers", async () => {
    ...
    await expect(
      filter.filterMultiaddrForPeer(blockedPeerId)
    ).resolves.toBeFalsy();
  });
});
