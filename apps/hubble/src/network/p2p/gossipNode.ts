...
export class GossipNode extends TypedEmitter<NodeEvents> {
  ...
  async addPeerToAddressBook(peerId: PeerId, multiaddr: Multiaddr) {
    if (!this.addressBook) {
      ...
      const addResult = await ResultAsync.fromPromise(
        this.addressBook.save(peerId, { multiaddrs: [multiaddr] }),
        (error) => new HubError("unavailable", error as Error)
      );
      if (addResult.isErr()) {
        log.error(
          { error: addResult.error, peerId },
          "failed to add contact info to address book"
        );
      }
    }
  }
  ...
  async removePeerFromAddressBook(peerId: PeerId) {
    if (this._node) {
      ...
      if (hangupResult.isErr()) {
        log.error(
          { error: hangupResult.error, peerId },
          "failed to hang up peer"
        );
      }
    }
  ...
  async getPeerInfo(peerId: PeerId) {
    ...
    for (const conn of existingConnections ?? []) {
      ...
      if (
        knownAddrs?.addresses &&
        !knownAddrs.addresses.find((addr) =>
          addr.multiaddr.equals(conn.remoteAddr)
        )
      ) {
  ...
  async start(
    ...
    log.info(
      {
        ...
        addresses: this._node.getMultiaddrs().map((a) => a.toString()),
      },
      "Starting libp2p"
    );
    ...
    this._periodicPeerCheckJob = new PeriodicPeerCheckScheduler(
      this,
      bootstrapAddrs
    );
  ...
  async gossipContactInfo(
    contactInfo: ContactInfoContent
  ): Promise<HubResult<PublishResult>[]> {
    const gossipMessage = GossipMessage.create({
  ...
  async publish(message: GossipMessage): Promise<HubResult<PublishResult>[]> {
    ...
    log.debug(
      { identity: this.identity },
      `Publishing message to topics: ${topics}`
    );
    if (this.gossip === undefined) {
      return [
        err(
          new HubError("unavailable", new Error("GossipSub not initialized"))
        ),
      ];
    }
    ...
    if (encodedMessage.isErr()) {
      ...
      const results = await Promise.all(
        topics.map(async (topic) => {
          try {
            const publishResult = await gossip.publish(
              topic,
      ...
      log.debug(
        { identity: this.identity, results },
        "Published to gossip peers"
      );
      return results;
    }
  }
  ...
  async connect(peerNode: GossipNode): Promise<HubResult<void>> {
    ...
    return err(
      new HubError("unavailable", { message: "cannot connect to any peer" })
    );
  }
  ...
  async connectAddress(address: Multiaddr): Promise<HubResult<void>> {
    log.debug(
      ...
      `Attempting to connect to address ${address}`
    );
    try {
  ...
  registerListeners() {
    this._node?.addEventListener("connection:open", (event) => {
      log.info(
        {
    ...
    this._node?.addEventListener("connection:close", (event) => {
      log.info(
        { peer: event.detail.remotePeer },
    ...
    this.gossip?.addEventListener("gossipsub:message", (event) => {
      ...
      if (this.gossipTopics().includes(event.detail.msg.topic)) {
        this.emit(
          ...
          GossipNode.decodeMessage(event.detail.msg.data)
        );
      }
    });
  }
  ...
  registerDebugListeners() {
    this._node?.addEventListener("peer:discovery", (event) => {
      log.info(
        { identity: this.identity },
        `Found peer: ${event.detail.multiaddrs}  }`
      );
    });
    ...
    this._node?.addEventListener("connection:close", (event) => {
      log.info(
        { identity: this.identity },
        `Disconnected from: ${event.detail.remotePeer.toString()} `
      );
    });
  ...
  static decodeMessage(message: Uint8Array): HubResult<GossipMessage> {
    ...
    try {
      ...
      if (
        gossipMessage.topics.length === 0 ||
        supportedVersions.findIndex((v) => v === gossipMessage.version) === -1
      ) {
        return err(
          new HubError("bad_request.parse_failure", "invalid message")
        );
      }
  ...
  public async bootstrap(
    ...
    const results = await Promise.all(
      bootstrapAddrs.map((addr) => this.connectAddress(addr))
    );

    const finalResults = Result.combineWithAllErrors(results) as Result<
      void[],
      HubError[]
    >;
    if (
      finalResults.isErr() &&
      finalResults.error.length === bootstrapAddrs.length
    ) {
  ...
  public getMessageId(message: GossipSubMessage): Uint8Array {
    if (message.topic.includes(this.primaryTopic())) {
      ...
      if (
        protocolMessage.isOk() &&
        protocolMessage.value.version === GossipVersion.V1_1
      ) {
        if (protocolMessage.value.message !== undefined)
          return (
            protocolMessage.unwrapOr(undefined)?.message?.hash ??
            new Uint8Array()
          );
        if (protocolMessage.value.idRegistryEvent !== undefined)
          return (
            protocolMessage.value.idRegistryEvent?.transactionHash ??
            new Uint8Array()
          );
      }
    }
  ...
  private async createNode(options: NodeOptions): Promise<HubResult<Libp2p>> {
    ...
    if (options.announceIp && options.gossipPort) {
      const announceMultiAddr = addressInfoFromParts(
        options.announceIp,
        options.gossipPort
      ).map((addressInfo) => ipMultiAddrStrFromAddressInfo(addressInfo));
      if (announceMultiAddr.isOk() && announceMultiAddr.value.isOk()) {
        ...
        announceMultiAddrStrList = [
          `${announceIpMultiaddr}/tcp/${options.gossipPort}`,
        ];
      }
    }
